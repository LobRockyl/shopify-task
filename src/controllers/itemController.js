...
exports.createItem = async (req, reply) => {
  try {
    ...
    if(await sanityChecker.creation_data_checker(req.body)){
      ...
      reply.view('./src/views/index.ejs', {success:true,msg:"new item created"});
    }else{
      reply.view('./src/views/index.ejs', {success:false,msg:"Please input correctly"});
    }
...
exports.updateItem = async (req, reply) => {
  try {
    ...
    const checked_item_data = await sanityChecker.edition_data_checker_and_formatter(req.body)
    console.log(checked_item_data)
    if(checked_item_data.success==true){
      const { ...updateData } = checked_item_data.data
      const update = await Item.updateOne({_id:id}, updateData, { new: true })
      if (update.n > 0){
